...
# 频率统计使用的滑动窗口大小（使用最近N期来计算频率）
FREQUENCY_SLIDING_WINDOW = 50
# 机器学习模型训练使用的滑动窗口大小（使用最近N期来训练模型）
ML_TRAINING_SLIDING_WINDOW = 200
...
# ==============================================================================
# --- 可优化的滑动窗口参数 (这些参数可被Optuna优化) ---
# ==============================================================================
# 这些窗口参数决定了各分析组件使用的历史数据范围，影响时效性与统计稳定性的平衡
DEFAULT_WINDOW_PARAMS = {
    # 频率统计窗口：用于计算号码出现频率的期数范围
    'FREQUENCY_WINDOW': 50,
    # 模式分析窗口：用于分析和值、奇偶比、区间分布等模式的期数范围  
    'PATTERN_WINDOW': 50,
    # 关联规则窗口：用于挖掘号码关联规则的期数范围
    'ASSOCIATION_WINDOW': 50,
    # 机器学习训练窗口：用于训练预测模型的期数范围
    'ML_TRAINING_WINDOW': 200,
    # 近期频率窗口：用于计算"近期"活跃度的期数范围
    'RECENT_FREQ_WINDOW': 20,
}
...
def analyze_frequency_omission(df: pd.DataFrame, freq_window: int = FREQUENCY_SLIDING_WINDOW) -> dict:
    """
    分析所有号码的频率、当前遗漏、平均遗漏、最大遗漏和近期频率。
    使用滑动窗口来计算频率统计，更关注近期趋势。

    Args:
        df (pd.DataFrame): 包含历史数据的DataFrame。
        freq_window (int): 频率统计使用的滑动窗口大小，默认使用最近N期。
    ...
    # 使用滑动窗口计算频率（关键修改：只使用最近N期来计算频率）
    if total_periods >= freq_window:
        freq_df = df.tail(freq_window)  # 只取最近freq_window期的数据
        freq_periods = freq_window
        logger.debug(f"使用最近{freq_window}期数据计算频率统计")
    else:
        freq_df = df  # 如果总期数不足，使用全部数据
        freq_periods = total_periods
        logger.debug(f"数据不足{freq_window}期，使用全部{total_periods}期数据计算频率")
    
    # 基于滑动窗口的频率计算
    all_reds_flat = freq_df[red_cols].values.flatten()
    red_freq, blue_freq = Counter(all_reds_flat), Counter(freq_df['blue'])
    
    # 遗漏计算仍使用全部历史数据（保持原有逻辑）
    ...
    # 平均间隔计算基于滑动窗口的频率（关键修改）
    avg_interval = {num: freq_periods / (red_freq.get(num, 0) + 1e-9) for num in RED_BALL_RANGE}
    for num in BLUE_BALL_RANGE:
        avg_interval[f'blue_{num}'] = freq_periods / (blue_freq.get(num, 0) + 1e-9)
    ...
    return {
        ...
        'freq_window_used': freq_periods  # 新增：记录实际使用的窗口大小
    }

def analyze_patterns(df: pd.DataFrame, pattern_window: int = FREQUENCY_SLIDING_WINDOW) -> dict:
    """
    ...
    Args:
        ...
        pattern_window (int): 模式分析使用的滑动窗口大小，默认使用最近N期。
    ...
    # 使用滑动窗口分析模式（关键修改：只使用最近N期来分析模式）
    total_periods = len(df)
    if total_periods >= pattern_window:
        pattern_df = df.tail(pattern_window)  # 只取最近pattern_window期的数据
        logger.debug(f"使用最近{pattern_window}期数据分析模式趋势")
    else:
        pattern_df = df  # 如果总期数不足，使用全部数据
        logger.debug(f"数据不足{pattern_window}期，使用全部{total_periods}期数据分析模式")
    ...
    for col, name in [('red_sum', 'sum'), ('red_span', 'span'), ('red_odd_count', 'odd_count')]:
        if col in pattern_df.columns: res[f'most_common_{name}'] = safe_mode(pattern_df[col])
        
    zone_cols = [f'red_{zone}_count' for zone in RED_ZONES.keys()]
    if all(c in pattern_df.columns for c in zone_cols):
        dist_counts = pattern_df[zone_cols].apply(tuple, axis=1).value_counts()
        if not dist_counts.empty: res['most_common_zone_distribution'] = dist_counts.index[0]
        
    if 'blue_is_odd' in pattern_df.columns: res['most_common_blue_is_odd'] = safe_mode(pattern_df['blue_is_odd'])
    if 'blue_is_large' in pattern_df.columns: res['most_common_blue_is_large'] = safe_mode(pattern_df['blue_is_large'])
    
    return res

def analyze_associations(df: pd.DataFrame, weights_config: Dict, assoc_window: int = FREQUENCY_SLIDING_WINDOW) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        assoc_window (int): 关联规则分析使用的滑动窗口大小，默认使用最近N期。
    ...
    # 使用滑动窗口进行关联规则挖掘（关键修改：只使用最近N期来挖掘关联规则）
    total_periods = len(df)
    if total_periods >= assoc_window:
        assoc_df = df.tail(assoc_window)  # 只取最近assoc_window期的数据
        logger.debug(f"使用最近{assoc_window}期数据挖掘关联规则")
    else:
        assoc_df = df  # 如果总期数不足，使用全部数据
        logger.debug(f"数据不足{assoc_window}期，使用全部{total_periods}期数据挖掘关联规则")
    
    try:
        transactions = assoc_df[red_cols].astype(str).values.tolist()
...
def train_prediction_models(df_train_raw: pd.DataFrame, ml_lags_list: List[int], ml_window: int = ML_TRAINING_SLIDING_WINDOW) -> Optional[Dict[str, Any]]:
    """为所有红球和蓝球并行训练预测模型。使用滑动窗口来训练模型，关注近期趋势。"""
    
    # 使用滑动窗口选择训练数据（关键修改：只使用最近N期来训练模型）
    total_periods = len(df_train_raw)
    if total_periods >= ml_window:
        train_df = df_train_raw.tail(ml_window)  # 只取最近ml_window期的数据
        logger.debug(f"使用最近{ml_window}期数据训练机器学习模型")
    else:
        train_df = df_train_raw  # 如果总期数不足，使用全部数据
        logger.debug(f"数据不足{ml_window}期，使用全部{total_periods}期数据训练模型")
    
    if (X := create_lagged_features(train_df.copy(), ml_lags_list)) is None or X.empty:
        logger.warning("创建滞后特征失败或结果为空，跳过模型训练。")
        return None
        
    if (target_df := train_df.loc[X.index].copy()).empty: return None
...
def run_analysis_and_recommendation_with_windows(df_hist: pd.DataFrame, ml_lags: List[int], weights_config: Dict, window_params: Dict, arm_rules: pd.DataFrame) -> Tuple:
    """
    执行一次完整的分析和推荐流程，支持自定义滑动窗口参数。

    Args:
        df_hist: 历史数据DataFrame
        ml_lags: 机器学习滞后特征列表
        weights_config: 权重配置字典
        window_params: 滑动窗口参数字典
        arm_rules: 关联规则DataFrame

    Returns:
        tuple: 包含推荐组合、输出字符串、分析摘要、训练模型和分数的元组。
    """
    # 使用自定义窗口参数进行分析
    freq_data = analyze_frequency_omission(df_hist, window_params.get('FREQUENCY_WINDOW', FREQUENCY_SLIDING_WINDOW))
    patt_data = analyze_patterns(df_hist, window_params.get('PATTERN_WINDOW', FREQUENCY_SLIDING_WINDOW))
    ml_models = train_prediction_models(df_hist, ml_lags, window_params.get('ML_TRAINING_WINDOW', ML_TRAINING_SLIDING_WINDOW))
    probabilities = predict_next_draw_probabilities(df_hist, ml_models, ml_lags) if ml_models else {'red': {}, 'blue': {}}
    scores = calculate_scores(freq_data, probabilities, weights_config)
    recs, rec_strings = generate_combinations(scores, patt_data, arm_rules, weights_config)
    analysis_summary = {'frequency_omission': freq_data, 'patterns': patt_data}
...
def run_backtest_with_windows(full_df: pd.DataFrame, ml_lags: List[int], weights_config: Dict, window_params: Dict, arm_rules: pd.DataFrame, num_periods: int) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    在历史数据上执行策略回测，支持自定义滑动窗口参数。

    Args:
        full_df: 完整历史数据DataFrame
        ml_lags: 机器学习滞后特征列表
        weights_config: 权重配置字典
        window_params: 滑动窗口参数字典
        arm_rules: 关联规则DataFrame
    ...
    for i in range(num_periods):
        ...
        with SuppressOutput(suppress_stdout=True, capture_stderr=True):
            ...
            predicted_combos, _, _, _, _ = run_analysis_and_recommendation_with_windows(hist_data, ml_lags, weights_config, window_params, arm_rules)
...
def objective(trial: optuna.trial.Trial, df_for_opt: pd.DataFrame, ml_lags: List[int], arm_rules: pd.DataFrame) -> float:
    ...
    # 优化滑动窗口参数
    trial_window_params = {}
    for key, value in DEFAULT_WINDOW_PARAMS.items():
        if key == 'ML_TRAINING_WINDOW':
            # 机器学习训练窗口：确保足够的训练数据，范围较大
            trial_window_params[key] = trial.suggest_int(key, 150, 300, step=25)
        elif key == 'FREQUENCY_WINDOW':
            # 频率统计窗口：平衡时效性与统计稳定性
            trial_window_params[key] = trial.suggest_int(key, 30, 80, step=10)
        elif key == 'PATTERN_WINDOW':
            # 模式分析窗口：与频率窗口保持一致或略有差异
            trial_window_params[key] = trial.suggest_int(key, 30, 80, step=10)
        elif key == 'ASSOCIATION_WINDOW':
            # 关联规则窗口：需要足够数据挖掘规则，但不宜过大
            trial_window_params[key] = trial.suggest_int(key, 40, 100, step=10)
        elif key == 'RECENT_FREQ_WINDOW':
            # 近期频率窗口：应该较小以捕捉最新趋势
            trial_window_params[key] = trial.suggest_int(key, 10, 30, step=5)

    full_trial_weights = DEFAULT_WEIGHTS.copy()
    full_trial_weights.update(trial_weights)
    
    full_trial_window_params = DEFAULT_WINDOW_PARAMS.copy()
    full_trial_window_params.update(trial_window_params)
    
    # 在快速回测中评估这组权重和窗口参数
    with SuppressOutput():
        _, backtest_stats = run_backtest_with_windows(df_for_opt, ml_lags, full_trial_weights, full_trial_window_params, arm_rules, OPTIMIZATION_BACKTEST_PERIODS)
...
if __name__ == "__main__":
    ...
    active_window_params = DEFAULT_WINDOW_PARAMS.copy()
    optuna_summary = None
    ...
    if ENABLE_OPTUNA_OPTIMIZATION:
        ...
        # 优化前先进行一次全局关联规则分析
        optuna_arm_rules = analyze_associations(main_df, DEFAULT_WEIGHTS, DEFAULT_WINDOW_PARAMS.get('ASSOCIATION_WINDOW', FREQUENCY_SLIDING_WINDOW))
        ...
        try:
            ...
            weights_params = {k: v for k, v in best_params.items() if k in DEFAULT_WEIGHTS}
            window_params = {k: v for k, v in best_params.items() if k in DEFAULT_WINDOW_PARAMS}
            
            active_weights.update(weights_params)
            active_window_params.update(window_params)
            
            optuna_summary = {
                "status": "完成", 
                "best_value": study.best_value, 
                "best_weights": weights_params,
                "best_windows": window_params
            }
        except Exception as e:
            logger.error(f"Optuna 优化过程中断: {e}", exc_info=True)
            optuna_summary = {"status": "中断", "error": str(e)}
            logger.warning("优化中断，将使用默认权重和窗口参数继续分析。")
    ...
    logger.info(f"分析方法配置:")
    logger.info(f"  - 频率统计窗口: 最近 {active_window_params.get('FREQUENCY_WINDOW', FREQUENCY_SLIDING_WINDOW)} 期")
    logger.info(f"  - 模式分析窗口: 最近 {active_window_params.get('PATTERN_WINDOW', FREQUENCY_SLIDING_WINDOW)} 期")
    logger.info(f"  - 关联规则窗口: 最近 {active_window_params.get('ASSOCIATION_WINDOW', FREQUENCY_SLIDING_WINDOW)} 期")
    logger.info(f"  - 机器学习窗口: 最近 {active_window_params.get('ML_TRAINING_WINDOW', ML_TRAINING_SLIDING_WINDOW)} 期")
    ...
    if ENABLE_OPTUNA_OPTIMIZATION and optuna_summary:
        ...
        if optuna_summary['status'] == '完成':
            ...
            best_params_str = json.dumps(optuna_summary['best_weights'], indent=2, ensure_ascii=False)
            logger.info(best_params_str)
            logger.info("--- 本次分析已采用以下优化窗口参数 ---")
            window_params_str = json.dumps(optuna_summary['best_windows'], indent=2, ensure_ascii=False)
            logger.info(window_params_str)
        else: logger.info(f"错误信息: {optuna_summary['error']}")
    else:
        logger.info("\n--- 本次分析使用脚本内置的默认权重 ---")

    # 6. 全局分析
    full_history_arm_rules = analyze_associations(main_df, active_weights, active_window_params.get('ASSOCIATION_WINDOW', FREQUENCY_SLIDING_WINDOW))
    
    # 7. 回测并打印报告
    logger.info("\n" + "="*25 + " 策 略 回 测 摘 要 " + "="*25)
    backtest_results_df, backtest_stats = run_backtest_with_windows(main_df, ML_LAG_FEATURES, active_weights, active_window_params, full_history_arm_rules, BACKTEST_PERIODS_COUNT)
    ...
    final_recs, final_rec_strings, _, _, final_scores = run_analysis_and_recommendation_with_windows(main_df, ML_LAG_FEATURES, active_weights, active_window_params, full_history_arm_rules)
    ...
    # 9. 微信推送
    try:
        from ssq_wxpusher import send_analysis_report, send_complete_recommendations
        logger.info("正在发送微信推送...")
        
        # 准备复式参考数据
        complex_red_data = None
        complex_blue_data = None
        if final_scores and final_scores.get('red_scores'):
            top_7_red = sorted([n for n, _ in sorted(final_scores['red_scores'].items(), key=lambda x: x[1], reverse=True)[:7]])
            top_7_blue = sorted([n for n, _ in sorted(final_scores['blue_scores'].items(), key=lambda x: x[1], reverse=True)[:7]])
            complex_red_data = [f'{n:02d}' for n in top_7_red]
            complex_blue_data = [f'{n:02d}' for n in top_7_blue]
        
        # 发送分析报告推送（包含摘要信息）
        push_result1 = send_analysis_report(
            report_content="",  # 不需要完整报告内容
            period=last_period + 1,
            recommendations=final_rec_strings[:5],  # 只显示前5注作为摘要
            complex_red=complex_red_data,
            complex_blue=complex_blue_data,
            optuna_summary=optuna_summary,
            backtest_stats=backtest_stats
        )
        
        # 发送完整推荐号码推送
        push_result2 = send_complete_recommendations(
            period=last_period + 1,
            recommendations=final_rec_strings,  # 所有15注
            complex_red=complex_red_data,
            complex_blue=complex_blue_data
        )
        
        if push_result1.get('success') and push_result2.get('success'):
            logger.info("微信推送发送成功（分析报告+完整推荐）")
        elif push_result1.get('success') or push_result2.get('success'):
            logger.warning("微信推送部分成功")
        else:
            logger.warning(f"微信推送发送失败: {push_result1.get('error', '未知错误')}")
    ...
    # 10. 更新latest_ssq_analysis.txt
    try:
        with open(os.path.join(SCRIPT_DIR, 'latest_ssq_analysis.txt'), 'w', encoding='utf-8') as f:
            # 重新读取完整的日志文件内容
            with open(log_filename, 'r', encoding='utf-8') as log_f:
                f.write(log_f.read())
        logger.info("已更新 latest_ssq_analysis.txt")
    except Exception as e:
        logger.error(f"更新 latest_ssq_analysis.txt 失败: {e}")
